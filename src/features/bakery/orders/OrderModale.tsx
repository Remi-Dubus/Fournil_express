...
export default function OrderModale({
	...
	return (
		...
		>
			<section
				className={`overflow-y-scroll grid grid-cols-3 items-center w-11/12 gap-2 px-1 py-2 bg-light rounded-lg shadow-lg transform duration-1000 ease-in-out xl:w-1/3 ${openModale ? "translate-y-0" : "translate-y-full"}`}
			>
				...
				{selectedOrder?.products.map((p) => (
					<ul key={p.id} className="text-dark col-span-3 grid grid-cols-3">
						<li className="break-words">{p.label}</li>
